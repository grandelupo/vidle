  ...
  $(function() {
    ...
    ifExists('#contact .contact-section', function() {
      $('.contact-form').on('submit', function(event) {
        ...
        submitBtn.html('Wczytywanie...').addClass('wait').prop('disabled', true);
        setTimeout(function() {
          ...
          $.ajax({
            ...
            .done(function(response) {
              ...
              if (response.action == 'redirect') {
                ...
                submitBtn.removeClass('wait').html('Przekierowywanie...').addClass('success');
                feedbackEl.addClass('success').html(response.message).fadeIn(200);
            ...
            .fail(function(response) {
                ...
                feedbackEl.addClass('error').html('');

                if (response.responseJSON?.errors?.password !== undefined) {
                  feedbackEl.html(response.responseJSON.errors.password).fadeIn(200);
                }
                if (response.responseJSON?.errors?.email !== undefined) {
                  feedbackEl.html(response.responseJSON.errors.email).fadeIn(200);
                }
                if (response.responseJSON?.errors?.name !== undefined) {
                  feedbackEl.html(response.responseJSON.errors.name).fadeIn(200);
                }

                submitBtn.html(submitBtnText).removeClass('wait').prop('disabled', false);
            });
        }, 1000);
      });
    });
